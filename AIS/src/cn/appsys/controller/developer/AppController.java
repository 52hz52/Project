...
public class AppController {
	...
	public String getUserinfo(String code ,HttpSession session,HttpServletRequest request) {
		 DevUser devUser = null;
		// code =  request.getParameter("devCode");
		 //controller 中取得其他Controller中保存的Session 需要加 getSession
		 code = (String)request.getSession().getAttribute("devCodeU");
		
		 System.out.println("---------------app use code = "+code);
		try {
			devUser = devUserService.UserInfo(code);
			session.setAttribute("devUesr", devUser);
	...
	public String updateUserInfo(@RequestParam("kuserName")String kuserName,
			@RequestParam("kuserCode")String kuserCode,
			@RequestParam("kuserPassword")String kuserPassword,
			@RequestParam("kuserEmail")String kuserEmail,
			@RequestParam("kuserInfo")String kuserInfo,
			HttpSession session,
			HttpServletRequest request
			) {
		DevUser dv = (DevUser)session.getAttribute("devUesr");
		
		DevUser devUser = new DevUser();
		devUser.setId(dv.getId());
		devUser.setDevCode(kuserCode);
		devUser.setDevName(kuserName);
		devUser.setDevPassword(kuserPassword);
		devUser.setDevEmail(kuserEmail);
		devUser.setDevInfo(kuserInfo);
		int row = 0;
		try {
			row = devUserService.updateUserinfo(devUser);
			System.out.println("============="+row);
			getUserinfo(devUser.getDevCode() ,session, request);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
